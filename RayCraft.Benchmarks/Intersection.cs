...
namespace RayCraft.Benchmarks
{
    ...
    {
        private (int, int)[] hits2d = new (int, int)[64];
        private (int, int, int)[] hits3d = new (int, int, int)[64];
        ...
        public void BenchIntersect3d()
        {
            for (int x = 0; x < 1920; x++)
            {
                for (int y = 0; y < 1080; y++)
                {
                    Intersect3d(new Vector3(0.5f + x * 0.0025f, 82f, 0.5f + y * 0.0025f), new Vector3(1.0f, 0.5f, 0.5f), hits3d);
                }
            }
        }
